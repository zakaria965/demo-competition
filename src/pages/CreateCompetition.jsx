...
const CreateCompetition = () => {
    ...
    const handleRoundSelect = (selectedRound) => {
        ...
        // Abuurista meelo banaan oo loogu talagalay magacyada kooxaha
    ...
    const handleSubmit = (e) => {
        ...
        if (competitionName.trim() === '') {
            setError('⚠️ Fadlan gali magaca tartanka (competition name).');
            return;
        }
        if (round === '') {
            setError('⚠️ Fadlan dooro wareeg (round).');
            return;
        }
        
        // Sifee (Filter) oo khariidadee (map) magacyada kooxaha saxda ah
        ...
        // Hubinta tirada kooxaha
        if (teamsArray.length !== expectedCount) {
             setError(`⚠️ Waa in aad gelisaa ${expectedCount} magac oo kooxeed wareegga ${round} si sax ah.`);
             return;
        }

        // Hubi in su'aalo ay jiraan
        if (questionBank.length === 0) {
             setError('🛑 Question Bank-u waa madhan yahay! Fadlan ku shub su\'aalo inta aadan bilaabin tartanka.');
             return;
        }
        
        // Diyaari xogta kooxaha sida Array of Objects
